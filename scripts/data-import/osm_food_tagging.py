...
CHAINS = {
    ...
    "ペッパーランチ": ["#洋食", "#ステーキ"],
    "フォルクス": ["#洋食", "#ステーキ"],
    # ファストフード(バーガー/フライドチキン)は #洋食 や #ハンバーグ に含めない。
    #   ユーザー要望: 「洋食」検索でマック等が出ないように。CHAINSには載せず fallback(#お腹すいた)扱い。
    # 和食・牛丼・定食
    "吉野家": ["#和食"],
    "すき家": ["#和食"],
    "松屋": ["#和食"],
...
NAME_RULES = [
    ...
    (re.compile(r"家系|豚骨|とんこつ|こってり|二郎系|二郎インスパイア|背脂"), ["#ラーメン", "#こってりラーメン"]),  # 「二郎」単体は鳥二郎等の誤爆になるので系/インスパイアのみ（ラーメン二郎はCHAINSで拾う）
    ...
    (re.compile(r"洋食|欧風|グリル|ビストロ"), ["#洋食"]),  # 「レストラン/キッチン/食堂」等の汎用語は和食店等に誤爆するので外す
    # イタリアン
    ...
    # アジア系統
    (re.compile(r"インド料理|ネパール|インドカレー|タンドリー|ナンカレー|ナンセット"), ["#アジア系統", "#インドネパール料理"]),  # 「ナン」単体は『ナンバーワン』等に誤爆するので限定
...
# ファストフード（バーガー/フライドチキン）チェーン。#洋食/#ハンバーグ から除外する。
_FASTFOOD = re.compile(r"マクドナルド|モスバーガー|ロッテリア|ケンタッキー|バーガーキング|"
                       r"フレッシュネス|ファーストキッチン|ウェンディーズ|ドムドム|ゼッテリア")
...
def correct_baked_tags(name, tags):
    ...
    if "#洋食" in s and _ETHNIC_RESTAURANT.search(name) and (s & _ETHNIC_GENRE_TAGS):
        s.discard("#洋食")
    # ④ 「二郎」誤マッチ（鳥二郎=焼き鳥居酒屋 / 大二郎の小籠包=中華 等）が #こってりラーメン
    if "#こってりラーメン" in s and "二郎" in name \
            and not re.search(r"ラーメン二郎|ラーメン|らーめん|麺|つけ麺|二郎系|二郎インスパイア", name):
        s.discard("#こってりラーメン")
        if not re.search(r"ラーメン|らーめん|麺", name):
            s.discard("#ラーメン")
    # ⑤ 「ナンバー…」誤マッチ（長浜ナンバーワン=豚骨ラーメン 等）が #インドネパール料理
    if "#インドネパール料理" in s and re.search(r"ナンバー", name) \
            and not re.search(r"インド|ネパール|カレー|タンドリー", name):
        s.discard("#インドネパール料理")
        if not re.search(r"アジア|エスニック|タイ|ベトナム", name):
            s.discard("#アジア系統")
    # ⑥ ファストフード(バーガー/フライドチキン)は #洋食/#ハンバーグ から除外（洋食検索にマック等を出さない）
    if _FASTFOOD.search(name):
        s.discard("#洋食"); s.discard("#ハンバーグ")
    # ⑦ 汎用語(レストラン/キッチン/食堂)由来の#洋食は、別の具体ジャンルがあるなら外す（和食/焼肉店等の誤爆解消）
    if "#洋食" in s and re.search(r"レストラン|キッチン|食堂|ダイニング", name) \
            and not re.search(r"洋食|欧風|グリル|ビストロ|ハンバーグ|オムライス|ステーキ|ハンバ", name) \
            and (s & {"#和食", "#中華", "#韓国", "#イタリアン", "#アジア系統",
                      "#インドネパール料理", "#タイ料理", "#ベトナム料理", "#焼肉", "#ラーメン",
                      "#居酒屋", "#カフェスイーツ", "#海鮮", "#うどんそば", "#お好み焼きもんじゃ"}):
        s.discard("#洋食")
    # ⑧ 店名が明確に焼肉系なら #洋食 を外す（OSM cuisine=steak併記で焼肉店が洋食化するのを防ぐ）
    if "#焼肉" in s and re.search(r"焼肉|カルビ|ホルモン|ジンギスカン", name):
        s.discard("#洋食")
...
if __name__ == "__main__":
    ...
    allok &= has({"amenity": "fast_food"}, "マクドナルド", ["#お腹すいた"], mustnot=["#洋食", "#ハンバーグ"])  # ファストフードは洋食扱いしない
    allok &= has({"amenity": "restaurant"}, "レストラン ひまわり", ["#お腹すいた"], mustnot=["#洋食"])  # 汎用レストランは洋食扱いしない（精度優先）
    ...
    allok &= has({"amenity": "restaurant"}, "鳥二郎 天王寺店", ["#居酒屋"], mustnot=["#こってりラーメン", "#ラーメン"])  # 焼き鳥居酒屋
    allok &= has({"amenity": "restaurant"}, "ラーメン二郎 三田本店", ["#ラーメン", "#こってりラーメン"])              # 本物の二郎(CHAINS)
    allok &= has({"amenity": "restaurant"}, "長浜ナンバーワン", [], mustnot=["#インドネパール料理"])                # 豚骨ラーメン(ナンバー誤爆)
    # ── 洋食の精度（ユーザー報告: 洋食検索に和食/マックが混入）────────────────────
    allok &= has({"amenity": "restaurant", "cuisine": "japanese"}, "和食レストラン さくら", ["#和食"], mustnot=["#洋食"])  # 汎用レストランで洋食誤爆しない
    allok &= has({"amenity": "fast_food"}, "モスバーガー 渋谷店", ["#お腹すいた"], mustnot=["#洋食", "#ハンバーグ"])
    allok &= has({"amenity": "restaurant"}, "グリル小宝", ["#洋食"])                                              # 本物の洋食は残る
    allok &= has({"amenity": "restaurant"}, "ハンバーグの店 はせ川", ["#洋食", "#ハンバーグ"])                       # ハンバーグ専門は残る
