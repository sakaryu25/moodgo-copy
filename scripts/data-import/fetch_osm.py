...
CATS=[
 ...
 ('leisure','amusement_arcade',["#わいわい楽しみたい","#ゲーセン"]),
 ('amenity','karaoke_box',["#わいわい楽しみたい","#カラオケ"]),
 ('tourism','theme_park',["#わいわい楽しみたい","#スリル味わいたい","#絶叫"]),
 ('leisure','water_park',["#わいわい楽しみたい","#スリル味わいたい"]),
 ('tourism','viewpoint',["#自然感じたい","#絶景","#スリル味わいたい","#高所"]),
]
def run(k,v):
...
for i,(k,v,tags) in enumerate(CATS):
    ...
    for el in d.get("elements",[]):
        ...
        if not any(0x3040<=ord(c)<=0x9fff or 0x30a0<=ord(c)<=0x30ff for c in name): continue  # 日本語名のみ
...
json.dump(recs,open("/tmp/osm_records.json","w"),ensure_ascii=False)
print("=== OSM合計",len(recs),"件 ===",flush=True)
