...
const PREF_PIN_POS: Record<string, { x: number; y: number }> = {
  ...
  "宮崎":   { x: 178, y: 650 },
  "鹿児島": { x: 60,  y: 768 },
  "沖縄":   { x: 176, y: 700 },   // 移動後の沖縄本島(下記REGION_VIEW.shift適用後)の上
  // 北海道・東北: 東北6県が縦一列に密集 → 日本海側/太平洋側の交互配置
  "北海道": { x: 395, y: 75 },
  "青森":   { x: 560, y: 225 },
  "秋田":   { x: 380, y: 285 },
  "岩手":   { x: 565, y: 300 },
  ...
  "宮城":   { x: 560, y: 368 },
  "福島":   { x: 548, y: 424 },
  // 中部: 9県が161×182に密集 → 周囲の海(日本海/太平洋)と縁へリング状に
  "石川":   { x: 258, y: 408 },
  "富山":   { x: 322, y: 380 },
  "新潟":   { x: 408, y: 366 },
  "福井":   { x: 250, y: 462 },
  ...
  "奈良":   { x: 300, y: 562 },
  "和歌山": { x: 242, y: 586 },
  // 中国: 横長5県 → 日本海側/瀬戸内側の沖合へ交互に
  "鳥取":   { x: 210, y: 476 },
  "島根":   { x: 128, y: 486 },
  "岡山":   { x: 204, y: 543 },
  "広島":   { x: 152, y: 553 },
  "山口":   { x: 94,  y: 566 },
  // 四国: 4県が87×73の小島に密集 → 島の四隅へ
  "香川":   { x: 192, y: 531 },
  "徳島":   { x: 216, y: 556 },
  "愛媛":   { x: 139, y: 549 },
  "高知":   { x: 178, y: 606 },
...
const REGION_VIEW: Record<string, {
  ...
  "九州・沖縄": {
    vb: { x: -50, y: 468, w: 280, h: 380 },   // 340→280幅: 九州本体を約21%さらに拡大
    shift: { "沖縄": { dx: -248, dy: -78 } }, // 沖縄は九州南東の空き海域へ(本島が右端で切れない位置)
  },
  "北海道・東北": {
    vb: { x: 337, y: -14, w: 347, h: 531 },   // デフォルト(y:-51)より窓を下げ、地図全体を上へ表示
  },
};
