// ── TabBar ─────────────────────────────────────────────────────────────────
// シンプルなガラスバー（Instagram / App Store 風）: ブラー＋単色アクセント
...
function TabItem({
  ...
  return (
    <TouchableOpacity onPress={handlePress} style={s.tab} activeOpacity={0.7}>
      <Animated.View style={[s.tabInner, { transform: [{ scale }] }]}>
        <Icon active={active} />
        <Text style={[s.label, { color: active ? ACTIVE : INACTIVE, fontWeight: active ? '600' : '500' }]}>
          {label}
        </Text>
      </Animated.View>
    </TouchableOpacity>
