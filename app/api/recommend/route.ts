...
async function fetchYahooLocalSearch(
  ...
  bypassDisable: boolean = false,  // true=RECOMMEND_DISABLE_GOOGLE無視（チェーン支店展開のみYahoo無料を許可・ユーザー承認2026-07-25）
): Promise<ScoredItem[]> {
  if (RECOMMEND_DISABLE_GOOGLE && !bypassDisable) return [];   // 規約対応: 外部検索オフ時はYahoo検索も停止（チェーンのみ例外）
...
const GENZ_NOISE_RE = /銀行|信用金庫|信用組合|信金|労働金庫|ゆうちょ|郵便局|農協|JAバンク|(?:市|区|町|村|県)役所|役場|市役所|区役所|町役場|公民館|保健所|税務署|出張所|支所|地方合同庁舎|交番|派出所|駐在所|警察署|消防署|消防団|コインパーキング|タイムズ(?!スクエア)|月極|駐車場|バスターミナル|バスセンター|バス停|停留所|ガソリンスタンド|給油所|ENEOS|出光|コスモ石油|卸売|問屋|資材|建材|鋼材|製作所|加工場|工場|倉庫|物流|運送|運輸|事業所|営業所|協同組合|商工会|変電所|浄水場|下水|清掃工場|小学校|中学校|中学$|高等学校|高校|幼稚園|保育園|保育所|こども園|学童|児童公園|児童遊園|ちびっ子(?:広場|公園)|防災(?:公園|広場|拠点)|街区公園|近隣公園|歩道橋|陸橋(?!.*(?:カフェ|バル))|自動販売機|コインランドリー|質屋|閉店(?:しました)?$|閉館中|休館中|(?:^|[^年])休業中|廃業|営業終了|営業を終了|移転しました/;
// 外国人観光客向けの「写真/待ち合わせだけ」のランドマークを除去(2026-07-20 ユーザー要望「ハチ公前など外人だけ良い印象」)。
//   地元Z世代は“そこで何かする”場所を求めるため、活動性の無い銅像/交差点/モニュメント/発祥の地を消す。
//   実際に楽しめる名所(鎌倉大仏/スカイツリー/清水寺/展望タワー等)は誤爆しない(テスト済)。
const TOURIST_TRAP_RE = /ハチ公|忠犬ハチ|モヤイ像|スクランブル交差点|(?:^|[^ん])交差点|銅像|記念像|(?:平和|自由|乙女|裸婦|少年|勝利)の像|モニュメント|発祥(?:の|之)地|ゆかりの地/;
...
出力は {"order":[全番号を1回ずつ], "reject":[場違いな番号のみ・無ければ空], "reasons":{"番号":"その人向けの1行理由"}} のみをJSONで。order には全番号を必ず1回ずつ含める（rejectした番号も order には残す）。reasons は order 上位15件のみでよい。` },
        ...
        if (chainSeeds.length > 0 && hasLocation && !isProprietaryOnly) {
          try {
            ...
            for (const seed of chainSeeds.slice(0, 3)) {
              ...
              try { items = await fetchYahooLocalSearch(_oLat, _oLng, radiusKm, seed.query, answers, _tc, false, true); } catch { items = []; }  // openOnly=false(深夜可)・bypassDisable=true(チェーンのみ外部許可)
              ...
              for (const { it, km } of withKm) {
                ...
                chainRecs.push({
                  ...
                  _rawSource: "admin",  // 最終段の _pull/finalize で curated 扱い＝押し出されず確保される
                  isUserSpot: false,
                  hasUserPhotos: false,
                  userPhotoCount: 0,
                  routesByMode: undefined,
        ...
        {
          const genzKept = recommendations.filter(r => {
            const t = (r.title ?? "").trim();
            return !GENZ_NOISE_RE.test(t) && !TOURIST_TRAP_RE.test(t);
          });
