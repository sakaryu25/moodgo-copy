...
- **【お腹すいた】専用ルール（気分が「お腹すいた」の場合は必須）**:
  ...
  - 【食べたいジャンルは？】の回答を全クエリの先頭に必ず反映する（例：「ラーメン🍜」→ 全クエリに「ラーメン」を含める）
  - 【今の空腹度は？】の回答を量・スタイルに反映する（「軽く食べたい」→軽食/テイクアウト優先、「ドカ食いしたい」→食べ放題/大盛り優先）
  - 【お店の雰囲気は？】の回答をクエリに反映する（「賑やか」→大衆的・活気ある、「静か」→隠れ家・個室、「おしゃれ」→インスタ映え、「密室」→完全個室）
  - 【どんなペースで食べたい？】の回答をクエリに反映する（「サクッと」→ファストフード/丼/ラーメン、「座ってゆっくり」→レストラン/定食、「食べ放題」→バイキング/食べ放題店）
  - ジャンル×空腹度×雰囲気×ペースを組み合わせた複合クエリを生成し、ユーザーの具体的ニーズにマッチした飲食店のみを提案すること
