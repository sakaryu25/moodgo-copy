...
const GENRE_NEGATIVE_RE: Record<string, RegExp> = {
  ...
  // 運動: 外でひろびろ（ゴルフ練習場・墓地を除外）
  "外でひろびろ":       /ゴルフ練習|打ちっぱなし|ゴルフ場|墓地|霊園|駐車場|遊具|児童/i,
...
const FOOD_OK_DEEPDIVE_RE = /カフェ|喫茶|スイーツ|パフェ|フルーツ|グルメ|ご当地|道の駅|食べ歩き|ファミレス|パンケーキ|ブランチ|ランチ|ディナー/i;
function isFoodAllowedContext(mood: string | undefined, deepDive: string | undefined): boolean {
...
function tagsAreFood(tags?: string[]): boolean {
    ...
    try {
      ...
      if (sbResults.length >= 1 || hasLocation) {
        ...
        const finalizeSource = (arr: Rec[]): Rec[] => {
          const sorted = sortOrShuffle(nonFoodSanitize(genreFidelityFilter(qualitySanitize(seenFilter(foodSanitize(applyMallFilter(arr)))))));
        ...
        if (recommendations.length < 15 && hasLocation) {
          ...
          if (hasGenreDef || effectiveDeepDive) {
            ...
            const genrePool = sortOrShuffle(nonFoodSanitize(genreFidelityFilter(qualitySanitize(seenFilter(foodSanitize(applyMallFilter([...gGenre, ...yGenre] as Rec[])))))));
