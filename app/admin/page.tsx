...
function PhotoHarvestPanel({ secret }: { secret: string }) {
  ...
  const TYPES: [string, string][] = [
    ...
    // 追加分（索引を増やすだけ＝照合ロジックは不変なので誤爆リスクは増えない）。
    //   自前DBに多い「島/岬/峠/古墳/城跡/道の駅/灯台/橋」を拾う。存在しないQIDは0件ログが出るだけで無害。
    ["Q23442", "島"], ["Q185113", "岬"], ["Q133056", "峠"], ["Q1141225", "古墳"],
    ["Q1187691", "道の駅"], ["Q39715", "灯台"], ["Q12280", "橋"], ["Q39816", "谷"],
  ];
                          ...
                          <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "4px" }}>
                            <a
                              href={p.google_place_id
                                ...
                                : `https://www.google.com/search?q=${encodeURIComponent(p.name + " " + (p.address ?? ""))}`}
                              target="_blank"
                              rel="noreferrer"
                              style={{ fontWeight: 800, fontSize: "13px", color: "#0891b2", flex: 1, textDecoration: "underline dotted", cursor: "pointer" }}
                              title="Googleで検索">
                              {p.name} 🔗
                            </a>
